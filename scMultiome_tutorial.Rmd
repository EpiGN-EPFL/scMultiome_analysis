...
Each feature (row) represents a genomic region (peak) in the format: `chromosome:start-end`. And we will notice that the matrix is still very big:

```{r}
dim(count_aggr$Peaks)
```

To **not kill this R session because of memory requirements**, we can do some filtering first:
