...
meta <- seurat@meta.data %>%
  mutate(stage=case_when(
    wsnn_res.0.4 %in% c(0, 4, 5) ~ 'telencephalon',
    wsnn_res.0.4 %in% c(1, 2, 6) ~ 'early',
    wsnn_res.0.4 %in% c(3, 7, 11) ~ 'nontelencephalon',
    wsnn_res.0.4 %in% c(10, 8, 9) ~ 'other',
    wsnn_res.0.4 %in% c(12) ~ 'neuron'
  )) %>%
...
top_ko_motif <- motif_enrichment_ko %>%
  filter(pvalue < 0.05 & fold.enrichment > 5) ##NOTE: you can choose the filter criteria
top_ko_motif
```
