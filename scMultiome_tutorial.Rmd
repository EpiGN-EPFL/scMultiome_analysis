...
```{r, results="markup"}
packageVersion("Seurat") # >5.0.0 is good
```
...
For this tutorial, we provide a subsetted data.

```{r}
count_aggr <- Read10X("data_small//filtered_feature_bc_matrix-small//")
```
...
```{r}
frag.file <- "data_small//atac_fragments.tsv.gz"
...
```{r}
seurat$nucleosome_group <- ifelse(seurat$nucleosome_signal > 0.5, 'NS > 0.5', 'NS < 0.5')
...
**NOTE**: There are indeed many other criteria and algorithms to evaluate the number of clusters. Here to make things simple, we just choose one resolution where the clusters are relatively stable.

```{r}
p1 <- DimPlot(seurat, group.by = "RNA_snn_res.0.4", 
...
seurat <- FindClusters(seurat, graph.name = "wsnn", resolution = 0.4, verbose = FALSE)
```

```{r}
p1 <- DimPlot(seurat,group.by = "sample",reduction = "wnn.umap") 
p2 <- DimPlot(seurat,group.by = "wsnn_res.0.4",label=T, reduction = "wnn.umap") & NoLegend()
p3 <- FeaturePlot(seurat,
                  marker_genes,
...
```{r}
# RNA-only clustering 
p1 <- DimPlot(seurat, group.by = "RNA_snn_res.0.4", reduction = "umap_rna", label = T) + 
  ggtitle("RNA-only Clustering")

# WNN clustering 
p2 <- DimPlot(seurat, group.by = "wsnn_res.0.4", reduction = "wnn.umap", label = T) + 
...
```{r}
table(RNA=seurat$RNA_snn_res.0.4, WNN=seurat$wsnn_res.0.4)  %>%
...
```{r}
DefaultAssay(seurat) <- "RNA"
DE_wnn <- wilcoxauc(seurat@assays$RNA$counts, y=seurat$wsnn_res.0.4)
...
DotPlot(seurat, df$marker_genes, group.by = 'wsnn_res.0.4') + RotatedAxis()
```
...
DotPlot(seurat, npc_markers, group.by = 'wsnn_res.0.4') + RotatedAxis()
```
...
avg <- AggregateExpression(seurat, assays = 'RNA', group.by='wsnn_res.0.4',
                           return.seurat=TRUE) 
...
meta <- seurat@meta.data %>%
  mutate(stage=case_when(
    wsnn_res.0.4 %in% c(0, 2) ~ 'telencephalon',
    wsnn_res.0.4 %in% c(1, 3, 4, 10) ~ 'early',
    wsnn_res.0.4 %in% c(6) ~ 'nontelencephalon',
    wsnn_res.0.4 %in% c(7, 9) ~ 'other',
    wsnn_res.0.4 %in% c(11) ~ 'neuron',
    wsnn_res.0.4 %in% c(5) ~ 'inhibitory_neuron',
    wsnn_res.0.4 %in% c(8) ~ 'excitatory_neuron'
  )) %>%
  mutate(celltype=interaction(stage, wsnn_res.0.4, drop = TRUE))
...
res_ko_vs_wt <- wilcoxauc(X=GetAssayData(seurat_sub), y = seurat_sub$group)
...
```{r}
res_ko_vs_wt_atac <- wilcoxauc(X=seurat_sub@assays$ATAC$counts, y = seurat_sub$group)
...
CoveragePlot(seurat_sub,
                   region = "chr5-171422932-171423823",
                   group.by = "sample",
                   extend.upstream = 5000,
                   extend.downstream = 5000)
```
...
top_ko_motif <- motif_enrichment_ko %>%
  filter(pvalue < 0.05 & fold.enrichment > 4) ##NOTE: you can choose the filter criteria
top_ko_motif
```
...
The enriched motifs in KO-specific accessible peaks may highlight transcription factors that are more active or act as upstream regulators of the gene expression programs altered by the knockout. If you’re interested, you can experiment with different filtering criteria to see if you can recover the motif of *GLI3*—the transcription factor that was perturbed in this experiment.
